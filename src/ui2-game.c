 ...
 *
 * Consider a two-paned layout for the command menus.
 */
...
static struct cmd_info *converted_list[KEYMAP_MODE_MAX][UCHAR_MAX + 1];
...
void cmd_init(void)
{
	const int mode_orig = KEYMAP_MODE_ORIG;
	...
	for (size_t j = 0; j < N_ELEMENTS(cmds_all) - 1; j++) {
		...
		/* Fill everything in */
		for (size_t i = 0; i < cmds_all[j].len; i++) {
			if (commands[i].key[mode_rogue] == 0) {
				/* If a roguelike key isn't set, use the standard one */
				commands[i].key[mode_rogue] = commands[i].key[mode_orig];
			}

			unsigned char key_orig = commands[i].key[mode_orig];
			unsigned char key_rogue = commands[i].key[mode_rogue];

			converted_list[mode_orig][key_orig] = &commands[i];
			converted_list[mode_rogue][key_rogue] = &commands[i];
		}
	}
}
...
unsigned char cmd_lookup_key(cmd_code lookup_cmd, int mode)
{
	...
	for (size_t i = 0; i < N_ELEMENTS(converted_list[mode]); i++) {
...
cmd_code cmd_lookup(unsigned char key, int mode)
{
	...
	if (converted_list[mode][key] == NULL) {
		return CMD_NULL;
	}
...
static bool cmd_prereq(const struct cmd_info *cmd)
{
	assert(cmd != NULL);
...
static bool cmd_get(const struct cmd_info **cmd, unsigned char *key, int *count)
{
	...
	*count = 0;
	...
	ui_event event = textui_get_command(count);
