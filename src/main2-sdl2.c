...
#define DEFAULT_KEY_REPEAT_RATE 40
...
#define DEFAULT_VERTICAL_TAB_BORDER 4
		...
		SDL_Rect dst)
{
	...
	{
		if (subwindow->index == DISPLAY_CAVE && dst_row > 0) {
			src.y -= src.h;
			src.h *= 2;
		...
		const struct term_hints *hints)
{
	...
	subwindow->full_rect.y = subwindow->window->inner_rect.y +
		(subwindow->window->inner_rect.h - subwindow->full_rect.h) / 2;
}
